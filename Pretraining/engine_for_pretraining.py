...
def train_one_epoch(model: torch.nn.Module, vqkd: torch.nn.Module,
    ...
    for step, (batch, extra_info) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        wandb.log({"epoch" :epoch, "loss" : loss , "accuracy_1" : mlm_acc_1 , "accuracy_2" : mlm_acc_2})
        ...
        wandb.log({"loss_scale" : loss_scale_value , "learning_rate" : max_lr , "grad_norm" : grad_norm , "weight_decay" : weight_decay_value})
        if log_writer is not None:
